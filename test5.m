...
for q = [12]
    for phi = AR
        load(sprintf('matfs/GarchWishartN%dQ%dRho%.3fEig-%.3f.mat', N, q, ...
                     rho, phi), 'ev');
        
        %% Find the largest component of the eigenvector
        load(sprintf('matfys/GarchWishartN%dQ%dRho%.3fX-%.3f.mat', ...
                     N, q, rho, phi), 'X');
        ...
        load(sprintf('GarchWishartN%dQ%dRatio-%.3f.mat', N, q, phi), ...
          'ratio');
        ...
        load(sprintf('GarchWishartN%dQ%dIPR-%.3f.mat', N, q, phi), ...
             'IPR');
...
plot(x3, polyval(P(1, :), x3), 'b');
plot(x4, polyval(P(2, :), x4), 'g');
hold off
...
P = NaN(2, 3);
plot(AR, fraction(1, :), 'b+-', AR, fraction(2, :), 'rx-');
...
plot((AR), (fraction(1, :)), 'k+', ...
     ...
     (AR), (fraction(2, :)), 'kx', ...
     (AR), (y(2, :)), 'g');
xlabel('Auto-correlation strength, \phi');
ylabel('Ratio of localized eigenvectors');
grid on
