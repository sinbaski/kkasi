...
as a returns model shall not be forgotten. In the next few sections,
we discuss some more realistic returns models.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
where $p$ and $q$ are constant integers; $\mu(t)$ is the mean process
and considered a small constant for intraday returns. $\epsilon_t$ is
called the innovation of the return and is a variate whose conditional
distribution \footnotemark is Gaussian and has variance $\sigma^2_t$:
\begin{eqnarray*}
...
It can be shown that the autocorrelation function of $\epsilon_t^2$ is
given by \cite{Bollerslev87}:
\begin{eqnarray*}
\text{corr}(\epsilon_t^2, \epsilon^2_{t-n}) &=& \sum_{i=1} ^{p \vee q}
(\alpha_i + \beta_i) \rho_{n-i} \text{ for $n > p$}
\end{eqnarray*}
...
\begin{equation}
  ...
  \int_{0}^h \sigma_{t-h+s}^2 ds = \sum_{k=1}^n \left(
    S_{t-h+kh/n} - S_{t-h+(k-1)h/n} \right)^2
\end{equation}
...
\S\ref{sec:volvo}. Another 5 intraday series are also studied and
collected in appendix \ref{chp:appendix2}.
...
First of all, to determine that the volatilities are auto-correlated
and/or that the squared returns and the volatilities are correlated, and
hence a GARCH(p, q) model is appropriate for the return series under
investigation, we plot the auto-correlation function (ACF) of the
squared returns. This is shown in figure
\ref{fig:nordea_15min_acf}. If the aforementioned features are absent
from the series, the auto-correlations are expected to be Gaussian
distributed with mean 0 and variance 1/T and hence mostly reside
within the confidence bounds set by $\pm 2/\sqrt{T}$
\cite{Bollerslev86, Bollerslev87}.
...
suggest a GARCH(p, q) model can be appropriate.
\begin{figure}[htb!]
  \centering
  \subfigure[]{
...
GARCH forecast is even worse than the sample mean. This is surprising
but not totally unexpected --- with only 3 parameters, the GARCH(1,1)
...
auto-correlations at several different time lags, the GARCH forecast
cannot be expected to have good accuracy.

In this particular case, we see that the ARIMA model has 3 relatively
...
structure, so the GARCH model cannot be expected to perform very
well. In contrast, the Volvo 15-minute returns studied in
\S\ref{sec:volvo} has more concentrated auto-correlations --- 0.12 and
0.06 at lags 2 and 3 --- thus the GARCH(1,1) model is also found to
perform better and even marginally better than the SV model for the
same series.
