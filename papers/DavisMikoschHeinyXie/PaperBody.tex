...
%% \section{Estimation of the largest eigenvalues: an overview in the iid case}\label{sec:motivation}
...
\section{General growth rates for $p_n$ in the iid heavy-tailed case}
\label{sec:Davis2}
% \setcounter{equation}{0}
This section is based on ideas in Heiny and Mikosch \cite{heiny:mikosch:2016} where one can also find detailed proofs.
...
and \eqref{eq:tc}.  Davis et al.~\cite{davis:mikosch:pfaffel:2016,davis:pfaffel:stelzer:2014}
and Heiny and Mikosch ~\cite{heiny:mikosch:2016} allowed for more general rates for
...
%In  Davis et al. \cite{davis:mikosch:pfaffel;2015} relation \eqref{eq:Davis2} was derived for $\alpha\in (0,1)$ even for \seq s
...
\subsubsection*{The matrix $\mathbf M(s)$} We introduce some auxiliary matrices
derived from the coefficient matrix $\H=(h_{kl})_{k,l\in \Z}$:
\beao
\H(s)=(h_{k,l+s})_{k,l\in \Z}, \qquad \mathbf M(s)= \H(0)\H(s)'\qquad s\ge 0\,.
\eeao
Notice that
...
\sum_{i=1}^\infty (v_i(s))^2 &=&  \frobnorm{\mathbf M(s)}^2= \sum_{i,j\in \Z} \sum_{l_1,l_2 \in \Z} h_{i,l_1} h_{j,l_1+s}h_{i,l_2}
h_{j,l_2+s}\nonumber\\
...
% \setcounter{equation}{0}
\subsection{Sample covariance matrices}\label{sec:samplecov}
...
\beao
\dfrac{\la_{(1)}-\la_{(2)}}{\la_{(1)}} \cid \dfrac{v_1-v_2}{v_1}\,\I_{\{U<(v_2/v_1)^{\alpha/2}\}}+(1-U^{2/\alpha})\,\I_{\{U\ge (v_2/v_1)^{\alpha/2}\}}\,.
\eeao
...
Accordingly, $\mathbf M(s)$ is either non-negative or non-positive definite. %This explains why a symmetrization of $\mathbf M(s)$ is irrelevant.
