...
Also note $g(\tilde \chi, \tilde \chi, \reals) = \img Z^2$,
...
\begin{enumerate}[(1)]
\item There exist random variables $0 < K_0 < K_1 < \dots$,
...
\item
  ...
  $(X_0, X_1, \dots, X_n, l_1, l_2, \dots, l_n)$. From now on we adopt
  the notation $\mathcal B(S)$ to denote the $\sigma$-field generated
  by the set $S$.
\end{enumerate}
...
+
\delta \I_{\mathcal C}(x) \nu(dy \times dl)
\]
Thus the MA-process regenerates only when it is in $\mathcal C$ and in
this case it regenerates with probability $\delta$. That is
\[
\P[(X_n, S_n) \text{ regenerates } | X_{n-1} = x] = \delta \I_{\mathcal C}(x)
\]
There is yet another useful property of the iid matrices $A_n$. Define
mapping
\[
A \cdotp x: (A, x) \in \img(A) \times \tilde \chi \to
\tilde \chi \ni {
  A x \over |A x|
}
\]
...
\[
\bar r_\xi = \sup_{x \in \tilde \chi} r_\xi(x),\quad
\underline r_\xi = \inf_{x \in \tilde \chi} r_\xi(x)
\]

There is also an eigen measure $\ell_\xi$ on $\mathcal B(\tilde \chi)$
...
$\ell_\xi r_\xi = \int_{\tilde \chi} r_\xi(x) \ell_\xi(dx) = 1$.
...
where $\text{dom}(A)$ denotes the domain of $A$. 
\[
\mu_A^\theta(S) = {
  \E [ \| A \|^\theta \I_S(A)]
  \over
...
\]
where $f$ is a function $f: \tilde \chi \to \reals$ and
$\eta$ is a probability measure on $\mathcal B(\tilde \chi)$.
By Lemma 2.2 of Collamore and Mentemeier
...
We start the process randomly, i.e. we simulate $V_0$ 

$\mathcal E_u$:
\begin{equation}
  \label{eq:fbf}
  \P(|V| > u) = \E_{\mathcal D} \mathcal E_u = \pi(\mathcal C) \E_{\mathcal D} \left[
    N_u \1{T_u < \tau} e^{-\xi S_{T_u}}
...
original kernel $P$ thereafter. The following notations are used:
\begin{eqnarray*}
...
\begin{lemma}
  ...
  \end{equation}
  then for all $\theta > 0$ satisfying
  $\frac{\E\|A\|^{\varphi + \theta}}{\E\|A\|^\varphi} < 1$ ,
  there exist $0 < b < 1$ and $M > 0$ such that
  \begin{equation}
    \label{eq:drift}
    \E_\varphi \left[\left.
...
\begin{proof}
  ...
  \[
  r_{\varphi, \theta}(x) = c \int_{\tilde \chi} \inn{x, y}^\theta
  ...
  \begin{eqnarray}
    ...
    &=&
    \1{|V_{n-1}| > M}
    \E_\varphi\left[\int_{\tilde \chi} \inn{V_n, y}^\theta d\ell^*_{\varphi,\theta}(y)|
      \mathscr F_{n-1} \right]
    \nonumber \\
    &=&
    \1{|V_{n-1}| > M}
    \E_\varphi\left[\int_{\tilde \chi} (\inn{A_n V_{n-1}, y} + \inn{B,
  ...
  \item if $\theta \leq 1$, by subadditivity we have
    \begin{eqnarray*}
      &&\E_\varphi\left[\int_{\tilde \chi} (\inn{A_n V_{n-1}, y} + \inn{B,
          y})^\theta d\ell^*_{\varphi,\theta}(y) | \mathscr F_{n-1} \right]\\
      &\leq& \E_\varphi\left[\int_{\tilde \chi} \inn{A_n V_{n-1}, y}^\theta
        d\ell^*_{\varphi,\theta}(y) | \mathscr F_{n-1} \right]
      + \E_\varphi\left[\int_{\tilde \chi} \inn{B, y}^\theta d\ell^*_{\varphi,\theta}(y) |
        \mathscr F_{n-1} \right] \\
      &=& \E_\varphi\left[|V_{n-1}|^\theta |A_n \tilde V_{n-1}|^\theta\int_{\tilde \chi}
    ...
    \]
    %% where
    %% $\bar r_{\varphi, \theta} = \sup_{x \in \tilde \chi} r_{\varphi, \theta}(x)$
    %% and
    %% $\underline r_{\varphi, \theta} = \inf_{x \in \tilde \chi} r_{\varphi, \theta}(x)$,
    we have $b < 1$ and \eqref{eq:drift} holds.
    %% \[
    %% \lambda_\varphi(\theta) \left[
    %%   1 +  
    %%   {|B|^\theta r_{\varphi,\theta}(\tilde B) \over M^\theta
    %%     \lambda_\varphi(\theta) r_{\varphi, \theta}(\tilde V_{n-1})}
...
\begin{remark}
  Suppose $|V_0| \leq M$. Iterating \eqref{eq:drift} yields
  \[
  \E_\varphi \left[
    |V_n|^\theta r_{\varphi, \theta}(\tilde V_n)
    \prod_{i=1}^{n-1}\1{|V_i| > M}\right]
  \leq b^{n-1} |V_1|^\theta r_{\varphi, \theta}(\tilde V_1) \1{|V_1| > M}
  \]
  But $\{\tau > n\} \subseteq \bigcap_{i=1}^{n-1}\{|V_i| > M\}$, So
  % $\{\tau > n\} \subseteq \{|V_n| > M\}$. So
  \[
  \E_\varphi \left[
    |V_n|^\theta r_{\varphi, \theta}(\tilde V_n)
    \1{\tau > n}
  \right]
  \leq
  b^{n-1} |V_1|^\theta r_{\varphi, \theta}(\tilde V_1) \1{|V_1| > M}
  \]
  Since $|V_n|\1{\tau > n} > M \1{\tau > n}$, we have
  \begin{eqnarray}
    \E_\varphi \left[
      M^\theta \underline r_{\varphi, \theta}
      \1{\tau > n}\right]
    &\leq&
    b^{n-1} |V_1|^\varphi \bar r_{\varphi, \theta}
    % \P_\varphi(\tau > n) &\leq& K b^{n-1} \nonumber \\
    % &=& K [b \lambda_{\beta}(\alpha)]^{n-1} \label{eq:ret_time}
  \end{eqnarray}
  Moreover,
  \[
  |V_1|^\varphi \leq \|A_1\|^\varphi |V_0|^\varphi + |B|^\varphi
  \]
...
\begin{proof}
  ...
  \begin{eqnarray}
    ...
    \label{eq:kioj}
  \end{eqnarray}
  For the 2nd term, we show in the following
  \begin{equation}
    \label{eq:frf}
