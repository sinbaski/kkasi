...
\begin{equation}
  ...
  \label{eq:bollerslev}
\end{equation}
Cf. Buraczewski et al \cite{buraczewski:damek:mikosch:2016}. In the
...
\[
\lim_{x \to \infty} x^{\xi} \P(\tilde y' V > x) = e_\xi(\tilde y)
\]
...
Although the probability $\P(\tilde y' V > x)$ has been given
aymptotically by the Kesten-Goldie theorem, one often wishes to know
...
the occurence of $\tilde y' V > x$ for a large $x$ is a rare event; a
...
$\P(\tilde y' V > x)$ or for the more general problem when the
