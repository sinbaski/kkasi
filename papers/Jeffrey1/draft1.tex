...
%   S_+^{d-1})$ such that
%   \begin{eqnarray*}
%     P_\alpha r_\alpha(x) &=& \lambda_{\alpha} r_\alpha(x) \\
%     l_\alpha P_\alpha (S) &=& \lambda_{\alpha} l_\alpha(S) \\
...
\begin{proof}
  ...
  \end{eqnarray*}
    ...
    \begin{eqnarray*}
      ...
      &\leq& 1 + \sum_{i=2}^\infty i^{2r} K \rho_\alpha^{i-2} < \infty
    \end{eqnarray*}
    ...
    \begin{eqnarray*}
      && \P(n \leq T_u < \tau) \\
      &\leq& \P(n < \tau) \\
      &\leq& K \rho_\alpha^{n-1}
    \end{eqnarray*}
    So we may write
    \[
    \E f_{u, n}(\xi) \leq c \lambda_{-s\xi}^{n/s} \rho_\alpha^{(n-1)/s}
    \]
    Because $\rho_\alpha$ can be chosen arbitrarily close but greater
    than $\lambda_\alpha < 1$, the assumption $\inf_{\alpha \in
      \mathbb R} \log \lambda_\alpha < -\log \lambda_{-(\xi+\epsilon)}$ implies
    $\alpha$ and $s$ can be chosen such that $\rho_\alpha <
    1/\lambda_{-s\xi}$. Then it follows
    \begin{eqnarray*}
      \sum_{n=1}^{\infty} [\E f_{u,n}(\xi)]^{1/\xi}  &\leq&
      c \rho_{\alpha}^{-1/s\xi}\sum_{n=1}^{\infty}
      \left[\rho_\alpha\lambda_{-s\xi}\right]^{n/s\xi} < \infty
    \end{eqnarray*}
    \item 
  \end{enumerate}
\end{proof}
