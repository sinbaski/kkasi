...
\begin{lemma}
  ...
  such that for all $\theta$ satisfying the condition above, the following
  inequality holds
  \begin{equation}
    \label{eq:drift}
    \E^\varphi \left[\left.
...
\begin{remark}
  ...
  \E^\varphi \left[
    ...
    \right]
  &\leq&
  b_{\varphi, \theta}^{n-1} \E^\varphi |V_{K_j + 1}|^\theta \bar r_{\theta}
  \end{eqnarray*}
  Moreover,
  \[
  \E^\varphi |V_{K_j + 1}|^\theta
  \leq
  \E^\varphi \|A_{K_j + 1}\|^\theta |V_{K_j}|^\theta + |B|^\theta
  \leq
  M_{\varphi, \theta}^\theta + |B|^\theta
  \]
  So we have
  \[
  \P_\varphi(K_{j + 1} - K_j > n-1)
  \leq
  b_{\varphi, \theta}^{n-1} \rho(\varphi, \theta)
  \]
  for a constant
  \[
  \rho(\varphi, \theta) = {
