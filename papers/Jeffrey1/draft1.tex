...
\begin{lemma}
  ...
  such that for all $X_{n-1} \in \sphere^{d-1}$
  and for all $\theta$ satisfying the condition above, the following
  inequality holds
  \begin{equation}
    \label{eq:drift}
...
\begin{proof}
  ...
  \begin{eqnarray*}
    ...
    + \E_{X_{n-1}}^\varphi\left[c(\theta)\int_{\sphere^{d-1}} \inn{B, y}^\theta \ell^*_{\theta}(dy) |
      \mathscr F_{n-1} \right] \\
    &=&
  ...
  Let
  \[
  b_{\varphi, \theta} = {1 \over \lambda(\varphi)}
  {\bar r_\varphi \over \underline r_\varphi}
  {\bar r_\theta \over \underline r_\theta}
  \E \| A \|^{\varphi + \theta}
  \left\{
  ...
  \right\}
  \]
  By the 2nd part of assumption \eqref{eq:drift_cond}, there exist
  $\theta > 0$ and $M$ sufficiently large such that
  $b_{\varphi, \theta} < 1$.
  Thus we have
  \[
  \E_{X_{n-1}}^\varphi
  ...
  r_\theta(\tilde V_{n-1})
  \]
  where $M_{\varphi, \theta}$ denotes the value of $M$ chosen
  according to $\varphi$ and $\theta$ such that
  $b_{\varphi, \theta} < 1$.
  \begin{case}
  ...
  \begin{eqnarray*}
    ...
    \left\{
      ...
      \left[
        {
          r_\theta(\tilde B) \lambda(\varphi) \underline r_\varphi \underline r_\theta
          \over
          \underline r_\theta \bar r_\varphi \bar r_\theta \E \|A\|^{\varphi + \theta} 
        }
      \right]^{1/\theta}
    \right\}^\theta
  \end{eqnarray*}
  ...
  \right\}^\theta
  \]
  with $M_{\varphi, \theta}$ chosen sufficiently large so that
  $b_{\varphi, \theta} < 1$.
\end{proof}
