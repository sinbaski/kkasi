...
\end{eqnarray*}
Clearly $(X_n, S_n)$ is a Markov additive process. Let $P$ denote the
kernel of this prcess. We have
\begin{eqnarray*}
...
Define mapping
\[
g: (x, y, l) \in \tilde \chi \times \tilde \chi \times \mathbb R
\to
\mathbb R_+ \ni {
  \inn{\vec e_{q+1}, e^l y}
  \over
...
Also note
$g(\tilde \chi, \tilde \chi, \mathbb R) = \supp Z^2$,
where by the short-hand notation $\supp Z^2$ we mean the support of
the probability measure of $Z^2$.
Choose a set $\mathcal C \subset \tilde \chi$ such that
\[
\inf_{w \in g(\mathcal C, \tilde \chi, \mathbb R)} f_{Z^2}(w) > 0
\]
We have
\begin{eqnarray*}
  P(x, dy \times dl) \geq
  \I_{\mathcal C}(x) \inf_{w \in g(\mathcal C, dy, dl)} g(w, dy, dl)
  | g(\mathcal C, dy, dl)| 
\end{eqnarray*}
where $|\cdot|$ denotes the Lebesgue measure.

Next we show that
this process has a regenerative structure. In view of Ney and Nummelin
\cite{ney:nummelin:1987}, lemma 3.1, this reduces to showing that the
transition kernel of $(X_n, S_n)$ satisfies a minorization condition.
