...
\begin{proof}
  ...
  \item We first consider the case $\lambda(-\xi\epsilon) <
    ...
    \item First of all, we prove $f_2(u,\xi) < \infty$. Minkowski's
      ...
      \item if $\xi < 1/2$, $s$ can be chosen from $(1, 1/2\xi)$ so
        that $(\cdot)^{1/2s\xi}$ is a convex functon. Moreover
        $\E_{-\xi(2s - 1)} \|A\| < \infty$ and $\E_{-\xi(2s - 1)} |B|
        < \infty$. Jensen's inequality gives
        \begin{eqnarray*}
          && [\E_{-\xi(2s - 1)} f_{2,n}(u,\xi)]^{1/2s\xi} \\
