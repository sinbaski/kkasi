...
\begin{proof}
  ...
  \left\{
    ...
    {|B|^\theta r_{\theta}(\tilde B) \lambda(\varphi) \underline r_\varphi \underline r_\theta
      \over
      M_{\varphi, \theta}^\theta \underline r_\theta \bar r_\varphi \bar r_\theta 
      \E \|A \|^{\varphi + \theta}
    }
  \right\}
  \]
  By the 2nd condition of assumption \eqref{eq:drift_cond}, one can
  choose $M_{\varphi, \theta}$ sufficiently large such that
  $b_{\varphi, \theta} < 1$.
  Thus we have
  \[
  \E^\varphi
