...
\end{equation}
for some function $e_\xi(\cdot) > 0$ on
...
operator $\mathscr P_\theta$ for $\theta \in \mathbb R$ and 
$f: \mathbb S^{d-1} \to \mathbb R$
\[
\mathscr P_\theta f(x) =
\E \left[
  |A x|^\theta f \left({A x \over |A x|}\right) 
\right]
\]
...
associated with $\lambda(\xi)$ i.e. $\mathscr P_\xi r_\xi(x) =  r_\xi(x)$.
...
r_\xi(x)^{-1}
r_\xi \left({A(w) x \over|A(w) x|}\right)
\]
where $\P_\xi$ denotes the shifted probability measure.
Once the threshold $u$ has been exceeded by $|V_t|$, we change the
transition kernel of the MA-process and hence the distribution of
$Z^2$ back to their original, since the process is recurrent under the
original kernel.
...
\end{eqnarray*}
In \S\ref{sec:consistency} we prove the relation \eqref{eq:fbf} and in
