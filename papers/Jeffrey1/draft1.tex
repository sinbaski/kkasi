...
\item
  \[
  \P[(X_{\tau_i}, l_{\tau_i}) \in \mathcal S \times \Gamma | \mathscr F_{\tau_i - 1}]
...
\begin{remark}
  ...
  \E_\varphi \left[
    \left.
    |V_n|^\theta r_{\varphi, \theta}(\tilde V_n)
    \prod_{i=1}^{n-1}\1{|V_i| > M}
    \right| \mathscr F_1
    \right]
  \leq b^{n-1} |V_1|^\theta r_{\varphi, \theta}(\tilde V_1)
  \]
  Because
  $\{K_1 > n - 1\} \subseteq \bigcap_{i=1}^{n-1}\{|V_i| > M\}$,
  \begin{eqnarray*}
  \E_\varphi \left[\left.
    |V_n|^\theta \underline r_{\varphi, \theta}
    \1{K_1 > n - 1}
    \right| \mathscr F_1
    \right]
  &\leq&
  b^{n-1} |V_1|^\theta \bar r_{\varphi, \theta} \\
  \E_\varphi \left[
    |V_n|^\theta \underline r_{\varphi, \theta}
    \1{K_1 > n - 1}
    \right]
  &\leq&
  b^{n-1} \E_\varphi |V_1|^\theta \bar r_{\varphi, \theta}
  \end{eqnarray*}
  Moreover,
  \[
  \E_\varphi |V_1|^\theta
  \leq
  \E_\varphi \|A_1\|^\theta |V_0|^\theta + |B|^\theta
  \leq
  M^\theta + |B|^\theta
  \]
  So we have
