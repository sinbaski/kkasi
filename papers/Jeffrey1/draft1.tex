...
\begin{lemma}
  ...
  \begin{equation}
    ...
    \right] < 1
  \end{equation}
  then there exist $0 < b(\varphi) < 1, M(\varphi) > 0$ such that
  \begin{equation}
    \label{eq:drift}
    \E_{X_{n-1}}^\varphi \left[\left.
...
\begin{proof}
  ...
  Proposition 3.1, an eigen function $r_\theta(\cdot)$ and an
  eigenmeasure $\ell_\theta(\cdot)$ exist for the operator
  $\mathscr P_\theta$. 
  In particular, the right eigen function can be represented as
  \[
  ...
  Thus we have
  \begin{eqnarray}
    && \E_{X_{n-1}}^\varphi \left[ |V_n|^\theta r_{\theta}(\tilde V_n)
    ...
    \E_{X_{n-1}}^\varphi
    \left[
      c(\theta)\int_{\sphere^{d-1}} \inn{V_n, y}^\theta d\ell^*_{\theta}(y)|
      \mathscr F_{n-1} \right]
    \nonumber \\
    &=&
    \1{|V_{n-1}| > M}
    \E_{X_{n-1}}^\varphi\left[c(\theta)\int_{\sphere^{d-1}} (\inn{A_n V_{n-1}, y} + \inn{B,
        y})^\theta d\ell^*_{\theta}(y) | \mathscr F_{n-1} \right]
  ...
  \item if $\theta \leq 1$, by subadditivity we have
    \begin{eqnarray*}
      &&\E_{X_{n-1}}^\varphi\left[c(\theta)\int_{\sphere^{d-1}} (\inn{A_n V_{n-1}, y} + \inn{B,
          y})^\theta d\ell^*_{\theta}(y) | \mathscr F_{n-1} \right]\\
      &\leq& \E_{X_{n-1}}^\varphi\left[c(\theta)\int_{\sphere^{d-1}} \inn{A_n V_{n-1}, y}^\theta
        d\ell^*_{\theta}(y) | \mathscr F_{n-1} \right]
      + \E_{X_{n-1}}^\varphi\left[c(\theta)\int_{\sphere^{d-1}} \inn{B, y}^\theta d\ell^*_{\theta}(y) |
        \mathscr F_{n-1} \right] \\
      &=&
      \E_{X_{n-1}}^\varphi\left[|V_{n-1}|^\theta |A_n \tilde V_{n-1}|^\theta
      ...
      %% r_{\theta}(\mathbf a \cdotp \tilde V_{n-1})
      %% d\mu_A^\varphi (\mathbf a) + |B|^\theta r_{\theta}(\tilde B) \\
      &=&
      |V_{n-1}|^\theta (
      \mathscr P_{\varphi, \theta}
      r_{\theta})(\tilde V_{n-1}) + |B|^\theta r_{\theta}(\tilde B) \\
      &=& |V_{n-1}|^\theta r_{\theta}(\tilde V_{n-1}) \lambda_\varphi(\theta)
      \left\{
        1 + 
        {|B|^\theta r_{\theta}(\tilde B)
          \over
    ...
    \end{eqnarray*}
    By lemma 2.2 of Collamore and Mentemeier
    \cite{collamore:mentemeier:2016}, $r_{\theta}$ is bounded
    from above and below. Therefore the last equation gives
    \begin{eqnarray*}
      && \E_{X_{n-1}}^\varphi \left[ \left.
        |V_n|^\theta r_{\theta}(\tilde V_n) \1{|V_{n-1}| > M}
        \right| \mathscr F_{n-1}
        \right] \\
      &<&
      |V_{n-1}|^\theta r_{\theta}(\tilde V_{n-1})
      \underbrace{
        \lambda_\varphi(\theta)
        \left[
          1 + 
      ...
      \1{|V_{n-1}| > M} \\
      &\leq&
      |V_{n-1}|^\theta r_{\theta}(\tilde V_{n-1}) b
    \end{eqnarray*}
    Note
    \[
    \lambda_\varphi(\theta)
    =
    \left(
    \E_{X_{n-1}}^\varphi \| A_n \cdots A_1 \|^\theta
    \right)^{1/n}
    \leq
    \left(
    \E_{X_{n-1}}^\varphi (\| A_1 \|^\theta)^n
    \right)^{1/n}
    =
    \E_{X_{n-1}}^\varphi \| A_1 \|^\theta  <  1
    \]
    If $M$ satisfies
    \[
    M >
    \left\{
     |B|^\theta r_{\theta}(\tilde B)
    \over
    [1 - \lambda_\varphi(\theta)] \underline r_{\theta}
    \right\}^{1/\theta}
    \]
...
\begin{remark}
  ...
  \E_{X_{n-1}}^\varphi \left[\left.
    ...
    \right]
  &\leq&
  b^{n-1} |V_{K_j + 1}|^\theta \bar r_{\theta} \\
  \E_{X_{n-1}}^\varphi \left[
    |V_{K_j + n}|^\theta \underline r_{\theta}
    \1{K_{j+1} - K_j > n - 1}
    \right]
  &\leq&
  b^{n-1} \E_{X_{n-1}}^\varphi |V_{K_j + 1}|^\theta \bar r_{\theta}
  \end{eqnarray*}
  Moreover,
  \[
  \E_{X_{n-1}}^\varphi |V_{K_j + 1}|^\theta
  \leq
  \E_{X_{n-1}}^\varphi \|A_{K_j + 1}\|^\theta |V_{K_j}|^\theta + |B|^\theta
  \leq
  M^\theta + |B|^\theta
  \]
  So we have
