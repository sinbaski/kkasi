...
\begin{equation}
  ...
  V_t = A_t V_{t-1} + B_t
\end{equation}
with $V_t$, $A_t$ and $B_t$ defined as above.
...
$\pi$ denote the unique stationary probability measure of
$V \overset{d}{=}V_n$, and $P_Z$ the probability measure of
$Z\overset{d}{=} Z_1$. Buraczewski et al
\cite{buraczewski:damek:mikosch:2016} (proposition 4.3.1) showed that
the support of $\pi$ is
\[
\chi = \overline{
  \left\{
  (I_{p+q-1} - a)^{-1} b:
...
\item If $p, q \geq 2$, there exists an non-empty open set $U \subset
  \text{supp } P_Z$.
\end{enumerate}
Before proceeding any further, it is worthwhile pointing out a few
...
\item \eqref{eq:garchpq} immediately implies
  ...
  $\P(Z_{n-1}^2 = x) = 0$ for all $x \in \text{supp } P_{Z_{n-1}^2}$.
...
\item \eqref{eq:bollerslev} implies the top Lyapunov exponent of $A_n$
  ...
  \]
  is negative and that the Markov chain $V_n$ has a
  unique stationary distribution $\pi$ with finite
  variance. Cf. Buraczewski et al
...
an aperiodic, positive Harris chain that is $\pi$-irreducible on $\chi$.

It follows from the Kesten-Goldie theorem
\[
\lim_{x \to \infty} x^{\xi} \P(\tilde y' V > u) = e_\xi(\tilde y)
\]
for some $\xi > 0$, some function $e_\xi(\cdot) > 0$ on $\mathbb S^{d-1}$
and all $\tilde y \in \mathbb S^{d-1}$. Here $d = p + q - 1$ and
$\tilde y'$ is the transpose of $\tilde
y$. Cf. Buraczewski et al \cite{buraczewski:damek:mikosch:2016}.
...
% aperiodic positive Harris chain on $\mathbb R_+^d = \{x \in \mathbb
% R^d: \forall i  = 1,\dots, d,\; \inn{e_i, x} \geq 0\}$. Here
% $\{e_1, \dots, e_d\}$ denote a orthonormal basis of $\mathbb R^d$ and
