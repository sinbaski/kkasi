...
$r_\xi$ is bounded from above and below by positive constants.

There is also an eigen measure $\ell_\xi$ on the $\sigma$-field
generated by $\tilde \chi$ associated with the operator
$\mathscr P_\theta$ that corresponds to the eigen value
...
until $|V_t| > u$. Let
\[
P_\xi(x, dy \times dl) = e^{\xi l} {r_\xi(y) \over r_\xi(x)} P(x, dy \times dl)
\]
Since the matrix $A_t$ depends only on $Z_{t-1}^2$, changing the
transition kernel of $(X_t, S_t)$ is equivalent to changing the
distribution of $Z_{t-1}^2$. It follows from the above equation
\[
{
  \P_\xi(Z_{t-1}^2 \in dw | X_{t-1} = x)
  \over
...
\]
where $\P_\xi$ denotes the shifted probability measure. Now we can
extend the definition of the operator $\mathscr P$ by replacing the
expectation taken with respect to the original measure with the
...
= \E_\varphi \left[
  |A x|^\theta f(A \cdotp x)
\right], \quad
\mu \mathscr P_{\varphi, \theta}(dx)
=
\E_\varphi \left[ |A x|^\theta \mu(A \cdotp dx) \right]
\]
where $f$ is a function $f: \tilde \chi \to \reals$ and
$\mu$ is a probability measure on the $\sigma$-field generated by
$\tilde \chi$.
By Lemma 2.2 of Collamore and Mentemeier
...
\begin{lemma}
  ...
  \begin{equation}
    \label{eq:drift}
    \E_\beta \left[\left.
        |V_n|^\alpha r_\beta(|\tilde V_n|; \alpha) \I_{{\mathcal S}^\complement}(V_{n-1}) \right|
      \mathcal F_{n-1} \right] \leq |V_{n-1}|^\alpha r_\beta(|\tilde
    V_{n-1}|; \alpha) \I_{{\mathcal S}^\complement}(V_{n-1})
  \end{equation}
  ...
  \]
  The set $\mathcal S = [-M_\beta(\alpha),
  M_\beta(\alpha)]$. $M_\beta(\alpha)$ is chosen with respect to the
  values of $\alpha$ and $\beta$.
\end{lemma}

\begin{proof}
  Condition \eqref{eq:drift_cond} implies
  $\E_\beta \|A\|^\alpha < \infty$.
  Then, using the results of Buraczewski and
...
Now, to prove \eqref{eq:drift}, consider
  ...
  \item if $\alpha \leq 1$, by subadditivity we have
    \begin{eqnarray*}
      ...
      \left\{
        ...
        {\E_\beta r_\beta(B_n; \alpha) \over |V_{n-1}|^\alpha \lambda_\beta(\alpha) r_\beta(\tilde V_{n-1}; \alpha)}
         \right\} \\
    \end{eqnarray*}
    When $V_{n-1} \in \mathcal S^\complement$, $|V_{n-1}| > M_\beta(\alpha)$.
    ...
    \begin{eqnarray*}
      && \E_\beta \left[ \left.
          |V_n|^\alpha r_\beta(\tilde V_n; \alpha) \I_{\mathcal S^\complement}(V_{n-1})
      ...
      \right]
      \I_{\mathcal S^\complement}(V_{n-1})
    \end{eqnarray*}
    Also note
    \begin{equation*}
    ...
    \end{equation*}
    Hence by 2nd assumption of \eqref{eq:drift_cond}, $\lambda_\beta(\alpha)
...
\begin{theorem}
  The estimator $\mathcal E_u$ is consistent, i.e. equation
  \eqref{eq:fbf} holds.
\end{theorem}
\begin{proof}
...
\begin{remark}
  ...
  \leq \rho_\beta(\alpha)^{n-1} |V_1|^\alpha r_\beta(|\tilde V_1|; \alpha) \I_{{\mathcal C}^\complement}(V_1)
  \]
  But $\prod_{i=1}^{n}\I_{{\mathcal C}^\complement}(V_i) = 1$ implies $\tau > n$, and in this
  case $|V_n| > M$, where
  \[
  M = \left[
