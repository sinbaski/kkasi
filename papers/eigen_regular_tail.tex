...
\section{A Proof}
Here we prove that, for a positive semidefinite $p \times p$ matrix
$\mtx M = (m_{ij})$, whose $k$th principle minor is denoted $M_k$ satisfies
\[
\lambda_1(\mtx M) \leq \lambda_1(M_{k}) + \sum_{i=k+1}^p m_{ii}
\]
\begin{proof}
  Because $M_k$ is positive semidefinite, we only need to prove the
  claim for $k=p-1$. If $m_{pp} = 0$, $\lambda_1(\mtx M) =
  \lambda_1(M_{k})$, the claim is true.

  Next we prove the claim when $m_{pp} > 0$ and $\mtx M_{p-1}$ is
  positive definite. For notational convenience, write $A$ for
  $M_{p-1}$ and $d$ for $m_{pp}$. We have
  \begin{eqnarray*}
    \mtx M &=&
    \begin{pmatrix}
      A & v \\
  ...
  \begin{eqnarray*}
    ...
    &\leq& \lambda_1(A) + d
  \end{eqnarray*}
  When $A$ is only semidefinite and not positive definite, we may
  write
  \[
  A = 
  Q \begin{pmatrix}
