...
u_{\text{max}}(\alpha, K)$ and hence $\hat\phi$ increases 
with $\alpha$ and decreases with $K$. This is a verification of Lemma
\ref{lemma:I}; however, when $\beta$ and $K'$ are equal to $\alpha$
and $K$, respectively, $\tilde u_{\text{max}}(\alpha, K)$ is not
monotone with respect to $\alpha$ or $K$. Rather, there is a most
preferred value of $\alpha$ for each value of $K$. Furthermore, this
most preferred value changes only slightly when the value of $K$
changes. Also notice that the value of $\xi$ does make a qualitative
difference in the behavior of $\tilde u_{\text{max}}(\alpha, K)$ and
$\hat\phi$, as is seen in the 3rd and the 4th plots of
\ref{fig:phi_hat_pareto}. So the power-form of the utility function
and the equality of the lower- and upper-tail-indices are
responsible for the observed behavior. If this form of utility
function is common among investors in a market, one can expect the 
equity return series in the market to have very similar tail indices.
\begin{figure}[htb!]
...
\begin{figure}[htb!]
  ...
  \caption{
   ...
   $\tilde u_{\text{max}}(\alpha, K)$, as a \fct\ of $\alpha$ and $K$
   in the two-sided Pareto model \eqref{eq:pareto} with $K'=0.012$,
   $\beta = 1.4$.
   Clearly, $\tilde u_{\text{max}}(\alpha, K)$ increases with $\alpha$
   and decreases with $K$ when $K'$ and $\beta$ are fixed.
   {\em The 3rd and 4th} graphs show
   $\tilde u_{\text{max}}(\alpha, K)$ as a \fct\ of $\alpha$
   and $K$ with $\beta = \alpha$ and $K' = K$.
   We choose the utility \fct\ $u$ from \eqref{eq:u} for $\xi = 1/2$
   and $\xi = 4$.
% $\tilde u_{\text{max}}(\alpha, K)$ for the power-utility \fct\
...
Thus by lemma \ref{lemma:I}, $\wt u_{\text{max}}$ is monotone
