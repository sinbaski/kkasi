...
\beam\label{eq:xxie1.0}
 \wt u(\phi,F_X)\nonumber
  ...
  \int_{0}^\infty
  u\big( (1 - \phi) \ex^r + \phi\, \ex^x \big)\,
  {1 + b \1(x \le  q) \over (K' + x)^{\beta + 1}} \,dx 
...
\begin{eqnarray*}
  ...
  - b u(\delta v) F_X(q)
\end{eqnarray*}
A detailed analysis of the montonicity of
\[
u(C(x)) \left[
  1 - {b \over 1 + b}\1_{\{x > q\}}
  \right]
+ u(C(-x))
\]
which comprises lemma \ref{lemma:III}, allows us to apply lemma
\ref{lemma:I} and conclude the following
\begin{itemize}
\item I am a good guy
\item So are you
\end{itemize}
...
\]
  ...
  \item If $\min\{1, \xi\} < a < \max\{1, \xi\}$, $y_\pm$ are not
    ...
    \item If in addition $\xi > 1$, i.e. $1 < a < \xi$, $(f - g)(y)$
      ...
      % conversely, $f(e^q) < g(e^q)$ implies $f(y) = g(y)$ must have a
      % solution on $(e^q, \infty)$, since \eqref{eq:f-g} tells us
      % $(f-g)(y) \to \infty$ as $y \to \infty$.

    \item If instead $\xi < 1$, i.e. $\xi < a < 1$, $(f - g)(y)$
      is monotonically decreasing.
    \end{enumerate}

  % \item If $a < \min\{1, \xi\}$ or $a > \max\{1, \xi\})$, ${\pd (f
  %     -g)(y) \over \pd y} = 0$ has 2 real solutions.
  % \end{enumerate}

  % \begin{enumerate}
  % \item If $a < 1$ and $\xi > 1$
  %   \begin{enumerate}
  %   \item when $x > q$, $U'(x)$ being monotonic is equivalent to ${\pd U'(x)
  %       \over \pd x}$ having no zero on $(q, \infty)$. 

  %     \begin{enumerate}
  %     \item If $f (e^q) > g(e^q)$ it is clear $f(y) = g(y)$
  %       has no solution on $(e^q, \infty)$.
  %     \item If $f (e^q) <  g(e^q)$ it is also clear $f(y) = g(y)$ must have
  %       one solution on $(e^q, \infty)$.
  %     \end{enumerate}
  %   \end{enumerate}
    
  %   \item If $a > 1$ and $\xi < 1$
  %     \begin{enumerate}
  %     \item when $x > q$, the same calculation that leads to
  %       \eqref{eq:fd} and \eqref{eq:gd} gives ${\pd f(y) \over \pd y}
  %       < 0$ and ${\pd g(y) \over \pd y} > 0$. So
  %     \begin{enumerate}
  %     \item If $f (e^q) >  g(e^q)$, $f(y) = g(y)$ must have
  %       one solution on $(e^q, \infty)$.
  %     \item If $f (e^q) < g(e^q)$, $f(y) = g(y)$ has no solution on $(e^q, \infty)$.
  %     \end{enumerate}
  %     \end{enumerate}
  ...
  %       \item when $x > q$
  %         \begin{enumerate}
  %         \item If $f (e^q) -  g(e^q) > 0$, we observe $f(y) - g(y)
  %           \to -\infty$ as $y \to \infty$. So by continuity $f(y) -
  %           g(y) = 0$ must have at least one solution on $(e^q, \infty)$.
  %         \end{enumerate}
  %       \end{enumerate}
  %     \item If $a > 1$ and $\xi > 1$
  %       \begin{enumerate}
  %       \item when $x > q$, both $f(y)$ and $g(y)$ decreases. Observe
  %         In this case, if $f(e^q) > g(e^q)$, there is no solution 
  %       \end{enumerate}
  \end{enumerate}
\end{proof}
