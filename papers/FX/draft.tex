...
an increasing function. For simplicity we assume the investor
...
\begin{equation}
  ...
  C = (1 - \phi) e^r + \phi e^X
\end{equation}
For the distribution of $X$, call it $F(\cdot)$, we assume
...
\begin{theorem} \label{thrm:I}
  ...
  < q$. Assume $q > 0$. Also assume $u(\cdot)$ is an increasing function.
  Then
  \[
  \td{G}{K} < 0 < \td{G}{\alpha}
  \]
...
\begin{proof}
  ...
  We have
    ...
    h = (e^{1/\alpha} - 1)K
    \]
    As for $\td{G(\alpha, \beta, K, K')}{K}$, by the same argument as for
    $\td{G(\alpha, \beta, K, K')}{\alpha}$, it suffices to show
    \[
    \pd{\mathcal U(F)}{K}(\alpha, \beta, K, K', \hat\phi) < 0
    \]
    We have
    \begin{eqnarray*}
      && \pd{\mathcal UF}{K}(\alpha, \beta, K, K', \hat\phi) \\
      &=&
      -{\alpha p (1+ b) \over \xi}
      \int_0^\infty
      \left[
      ...
      -{\alpha p (1+ b) \over \xi}      
      \left[
        (1 - \hat\phi) e^r + \hat\phi e^{-M/\alpha}
      \right]^{-\xi} \times \\
      &&
      \left\{
        \int_{K/\alpha}^{\infty}
...
% all agents guided by this class of utility
% functions will seek the largest $\alpha$ and the smallest $K$ found in
% a stock group e.g. a sector. 


% As a result, one can expect all the
% actively traded equities in the group to feature the same $\alpha$ and
% $K$. However, the estimated value of $K^\alpha$ may vary
% significantly. This can be understood as a result of the smallness of
% $K$, which as revealed in section \ref{sec:hybrid_estimation} is of the
% order of $10^{-2}$, and the inaccuracy of the estimated value of
% $\alpha$, which typically ranges from 2.5 to 5. Exponentiation hugely
% magnifies the inaccuracy in $\alpha$.
