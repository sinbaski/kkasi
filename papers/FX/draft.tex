...
Then we have 
\beao
\wt u(\phi,F_X) &=& \E [u(C)] + b\, \E \big[u(C)\1(C \le  \delta v)\big] - b \,u(\delta\, v) F_X(q)\,, %\label{eq:functional}
\eeao
...
Figure~\ref{fig:sectors_parameters}.
\par
We illustrate Lemma~\ref{thrm:I} in Figure~\ref{fig:preference_pareto}
for the power-utility \fct\ \eqref{eq:u}.
...
\begin{eqnarray}
  \wt u(\phi, F_X) &=& \alpha p K^\alpha (1 + b)
  \int_{0}^\infty {
    u(C(x)) \left[
...
+ u(C(-x))
\]
is not monotonic. To see this we may re-write the above as
\begin{equation}
U_{\text{all}} =
  -{\phi^{-\xi} \over \xi} \left\{
  \underbrace{
...
\begin{figure}[htb!]
  ...
  \end{minipage}
  \caption{The quantities $I(\alpha, K)$ for the power-utility \fct\
    with $\xi=1/2$ (left) and $\xi=4$ (right). The graphs clearly show
    that  $I(\alpha, K)$ increases with $\alpha$ and decreases with
    $K$.
  }
...
\begin{eqnarray}
  ...
  \label{eq:t1}
\end{eqnarray}
where $C(\cdot)$ is defined in \eqref{eq:xxie1}. As shown in the last
section, $U_{\text{all}}$ is not monotonic when $b > 0$ and $u(\cdot)$ 
takes the power-form of \eqref{eq:hjyr}; when $b = 0$, however,
$U_{\text{all}}$ is indeed monotonic depending on the values of $\xi$
and $(1 - \phi) e^r / \phi$. In the first case, we have to resort to
numerical methods; in the 2nd case, we can find some analytic results.
...
%% \begin{eqnarray*}
%%   \wt u(F, \phi) = \int_{0}^{\infty} e^{-b x} f(x, \alpha) dx
%% \end{eqnarray*}
%% Thus by the 1st case of lemma \ref{lemma:I} and lemma \ref{lemma:II},
%% $\opd{\alpha}\wt u(F, \phi) > 0$. In this extreme case, the investor
%% would rather seek the largest $\alpha$ or in other words, the lightest
%% tail in the market.
...
\phi$ and $G(\alpha)$ are monotonically decreasing with $\alpha$ as in the
...
\begin{figure}[htb!]
  ...
  \begin{minipage}{0.5\linewidth}
    \includegraphics[width=\textwidth]{U_b_t_power.pdf}
  \end{minipage}
  \caption{$\hat\phi$ and $G(\alpha)$ when the utility
...
\begin{lemma} \label{lemma:I}
  ...
  $\theta \in \Theta$ is defined on $x \in (a, b)$,
  ...
  Moreover, $f(x, \theta)$ satisfies
  \begin{equation}\label{eq:frt}
  \int_a^b {\pd f(x, \theta) \over \pd \theta} dx
  < \infty
  \end{equation}
  Assume function $h(\cdot)$ is defined on
  $(a, b)$ and is monotonic throughout this interval;
  $X \sim F$ and $\E h(X) < \infty$. Then
  \begin{enumerate}
...
\begin{proof}
  Firstly we show
  \[
    {\pd \over \pd \theta} \int_a^b f(x, \theta) dx
    =
    \int_a^b {\pd \over \pd \theta} f(x, \theta) dx    
  \]
  Let
  \[
  f_{\Delta}(x, \theta) = {
  ...
  \[
  \lim_{\Delta \to 0} \int_a^b f_\Delta(x, \theta) dx
  =
  \int_a^b {\pd f \over \pd \theta}(x, \theta) dx
  < \infty
  \]
  That is
  \[
  ...
  measure 0. Call this set $S_0$. We have
  \begin{eqnarray*}
    && {\pd \E h(X) \over \pd \theta} \\
    &=& \int_{(a,b) \setminus S_0} h(x)
    {\pd f \over \pd \theta}(x, \theta) dx \\
    &=& \underbrace{\int_{(a, x_0) \setminus S_0}
      h(x) {\pd f \over \pd \theta}(x, \theta) dx}_{I_1}
    + \underbrace{\int_{(x_0, b) \setminus S_0}
      h(x) {\pd f \over \pd \theta}(x,
      \theta) dx}_{I_2}
  \end{eqnarray*}
  Moreover, $x_0$ being located in the interior of $(a, b)$ and
  $h(\cdot)$ being monotonic implies $h(x_0) < \infty$.
  \begin{enumerate}
  \item When $h(x)$ is decreasing on $(a, b)$ and ${\pd f \over \pd
    \theta}(x, \theta) > 0$ on $(a, x_0)$
    \[
    I_1 > h(x_0) \int_{(a, x_0) \setminus S_0}
    {\pd f \over \pd \theta}(x, \theta) dx
    \]
    Similarly, because ${\pd f \over \pd \theta}(x, \theta) < 0$ for
    $x \in (x_0, b)$ and $h(x)$ is decreasing, we have
    \begin{eqnarray*}
      I_2 &=& \int_{(x_0, b) \setminus S_0} -h(x_0)
      \left|{\pd f \over \pd \theta}(x, \theta) \right| dx
      > -h(x_0)
      \int_{(x_0, b) \setminus S_0} \left| 
        {\pd f \over \pd \theta}(x, \theta)
      \right| dx
    \end{eqnarray*}
    Finally we have
    \begin{eqnarray*}
      {\pd \E h(X) \over \pd \theta}
      > h(x_0) \int_{(a, b) \setminus S_0}
      {\pd f \over \pd \theta}(x, \theta) dx
      = h(x_0) {\partial \over \partial \theta}
      \int_{(a, b) \setminus S_0} f(x, \theta) dx
      = 0
    \end{eqnarray*}
  \item If $h(\cdot)$ is increasing and $\exists x_0 \in (a, b)$ such that 
    ...
    \begin{eqnarray*}
      I_1 &=&
      \int_{(a, x_0) \setminus S_0} -h(x)
      \left| {\pd f \over \pd \theta}(x, \theta) \right| dx
      >
      -h(x_0) \int_{(a, x_0) \setminus S_0}
      \left| {\pd f \over \pd \theta}(x, \theta) \right| dx
    \end{eqnarray*}
    and obviously
    \[
    I_2 > h(x_0) \int_{(x_0, b) \setminus S_0}
    {\pd f \over \pd \theta}(x, \theta) dx
    \]
    So
    \[
    ...
    = I_1 + I_2
    > h(x_0) {\partial \over \partial \theta}
    \int_{(a, b) \setminus S_0} f(x, \theta) dx = 0
    \]
  \end{enumerate}
\end{proof}
