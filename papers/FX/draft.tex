...
\subsubsection{mildly risk-avert utility function}
...
\begin{eqnarray}
  ...
  &&
  + \sum_{k=0}^\infty
  {\alpha p \over M^{k+1} ((1 - \phi) e^r)^{\gamma - 1}}
  \prod_{i=0}^{k-1} (\alpha + i)
  ...
  \label{eq:xxie1}
\end{eqnarray}
Observe that the sum over $n$ does not involve $\alpha$. In the
following we show this sum is positive for every $k \geq 0$, then it
is clear from \eqref{eq:xxie1} that $I_3$ and hence
$u(v) + b u(\delta v)$ is monotonically increasing with $\alpha$. To
see this, denote
\[
a_n(k) =
{(-1)^{n-1} \over n! n^{k+1}}
\left(
     ...
     \prod_{i=1}^{n-1} (\gamma + i - 1)
\]
For each given $k$, $a_n(k)$ is an alternating series. To show the
positivity of $\sum_n a_n(k)$, it suffices to show
\[
  {a_n(k) \over |a_{n+1}(k)|} > 1
  \quad
  \text{ for } n=1,3,5,...
\]
...
Thus we see that
$u(v) + u(\delta v)$ is monotonically increasing with $\alpha$. If
all investors are guided by and hence seek to maximize the same
utility function, they would all have constructed their portfolios by
choosing from a set of equities that have tail indices close to the
largest in the market. At market equilibrium, all actively traded
equities would have tail indices that are indistinguishable from each
other.

Notice that the summand of \eqref{eq:xxie1} decreases 
