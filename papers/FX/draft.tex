...
considerably riskier than the ``Consumer Staples'' sector. {\red Let
  Casper decide about this sentence.} This is of course a confirmation
of one's economic instinct.
...
\end{table}
We also acknowledge that there is yet another issue concerning the use
of $T_n$ as a test for a change in the tail-index: It is designed as a
test for a change in an extreme quantile and hence is not invariant
under permutation of the observations. This is clear from
\eqref{eq:4}. If the observations are iid, the null hypothesis is of
course not rejected with or without permutation; If instead the
observations are not iid but rather have small changes of the
tail-index over time, the null hypothesis may be rejected with
permutation but not rejected without. The latter may happen when the
changes in the tail-index are small but just big enough for the
accumulated change between two time points to exceed the given
critical value. In other words, the test cannot detect slow changes.

We illustrate this point in figure \ref{fig:PairTest:permuted} where
we apply a random permutation of observations to each series before
concatenating them and running the test.
\begin{figure}[htb!]
  \begin{minipage}{0.33\linewidth}
    \includegraphics[
      width=\textwidth,
...
are degenerate since they do not allow for a linear relationship
...
+ u(C(-x))
\]
is not monotone.\footnote {
To see this we may plug \eqref{eq:hjyr} in $U_{\text{all}}$ and
re-write it as
\[
U_{\text{all}} =
  -{\phi^{-\xi} \over \xi} \left\{
...
${1 \over \xi} {\pd U(x) \over \pd x} = {x' \over a^{\xi + 1}} + o(x')$
...
$
So $U(x)$ and hence $U_{\text{all}}$ is not monotone.}
So, unlike the case when $\beta$ and $K'$
are independent of $\alpha$ and $K$, we cannot use Lemma \ref{lemma:I}
to determine the monotonicity of $\tilde u_{\rm max}(\alpha, K)$.
Instead, we resort to numerical methods to gain some understanding of
...
\eqref{eq:u}. The corresponding $\tilde u_{\rm max}(\alpha, K)$
is shown in figure \ref{fig:preference_pareto}.
One can see that, when $K'$ and $\beta$ are fixed, $\tilde
u_{\rm max}(\alpha, K)$ and $\hat\phi$ increase 
with $\alpha$ and decreases with $K$. This is a verification of Lemma
\ref{lemma:I}; however, when $\beta$ and $K'$ are equal to $\alpha$
and $K$, $\tilde u_{\rm max}(\alpha, K)$ is monotone
decreasing with $\alpha$ but is rather insensitive to $K$. On the
other hand, $\hat\phi$ is not monotone with respect to $\alpha$ or
$K$. For each fixed $K$, it peaks at an $\alpha$ value somewhere below
1. For typical realistic $\alpha$ values in $(2, 4)$, $\hat\phi$ is a
small value below 5\%.
Since $\tilde u_{\rm max}(\alpha, K)$ is monotone decreasing with
$\alpha$, investors who seek to maximize it will prefer the smallest
$\alpha$ in the market, resulting in similar values of $\alpha$ for
different equities.
\begin{figure}[htb!]
 ...
 \caption{
   ...
   \eqref{eq:pareto} with $K'=0.012$, $\beta = 1.4$.
   {\em The 3rd and 4th} graphs show $\hat\phi$ as a \fct\ of $\alpha$
   and $K$ with $\beta = \alpha$ and $K' = K$.
   We choose the utility \fct\ $u$ from \eqref{eq:u} for $\xi = 1/2$
...
$U_{\text{all}}$ is monotone increasing or decreasing when $\xi$
...
it can be seen $\tilde u_{\rm max}(\alpha, K)$ is
...
decreasing when $b$ is small. We note that a sizable value of $b$
indicates a conservative, risk-averse investor.
...
\begin{figure}[htb!]
  ...
  %   \caption{$\hat\phi(\alpha)$ of log-utility function}
  %   \label{fig:phi_hat_t_log}
  % \end{subfigure}
  % \begin{subfigure}[b]{0.5\linewidth}
  %   \includegraphics[width=\textwidth]{U_b_t_log.pdf}
  %   \caption{$G(\alpha)$ of log-utility function}
  %   \label{fig:U_b_t_log}
  % \end{subfigure}
  % \begin{subfigure}[b]{0.5\linewidth}
  %   \includegraphics[width=\textwidth]{phi_hat_b_t_power.pdf}
  %   \caption{$\hat\phi(\alpha)$ of power-utility function}
  %   \label{fig:phi_hat_t_power}
  % \end{subfigure}
  % \begin{subfigure}[b]{0.5\linewidth}
  %   \includegraphics[width=\textwidth]{U_b_t_power.pdf}
  %   \caption{$G(\alpha)$ of power-utility function}
  ...
  \begin{minipage}{0.5\linewidth}
    \includegraphics[width=\textwidth]{U_b_t_log.pdf}
  \end{minipage}
  \caption{$\hat\phi$ and $\tilde u_{\rm max}(\alpha)$
    \label{fig:phi_hat_U}
  }
\end{figure}
...
\begin{proof}
  % Firstly we show, for any $S \subseteq (a, b)$,
  % \[
  ...
  % \[
  % f_{\Delta}(x, \theta) = {
  %   f(x, \theta +  \Delta) - f(x, \theta)
  %   \over
  %   \Delta
  % }
  % \]
  % Since $f(x, \theta)$ is differentiable for all $\theta \in \Theta$,
  % \[
  % f_{\Delta}(x, \theta) \to {\pd f \over \pd \theta}(x, \theta)
  % \quad \Delta \to 0
  % \]
  % For any give $\epsilon > 0$ and sufficiently small
  % $\Delta$, we have
  % \[
  % |f_{\Delta}(x, \theta)| <
  % \left|
  ...
  % \right| (1 + \epsilon)
  % \]
  % Using \eqref{eq:frt} and dominated convergence theorem we obtain
  % \[
  % \lim_{\Delta \to 0} \int_S f_\Delta(x, \theta) dx
  % =
  % \int_S {\pd f \over \pd \theta}(x, \theta) dx
  % < \infty
  % \]
  % That is
  % \[
  ...
  %   \int_S {\pd f \over \pd \theta}(x, \theta) dx     
  % \]
  % From \eqref{eq:frt} it easily follows that the set on which
  % ${\pd f(x, \theta) \over \pd \theta} = \infty$ has Lesbegue
  % measure 0. Call this set $S_0$.
...
\begin{proof}
  ...
  \end{eqnarray*}
  As for ${\pd \tilde u_{\rm max}(F_X) \over \pd K}$, by the same argument as for
  ${\pd \tilde u_{\rm max}(F_X) \over \pd \alpha}$, it suffices to show
  \[
