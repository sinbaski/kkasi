...
= 
\E u(C) + b \E [u(C); C < \delta v] - b u(\delta v) F(-q)
\]
In the rest of this section we write $q$ for $q(r, \phi)$
...
\begin{eqnarray}
  ...
  &&
  \underbrace{
    -b u(\delta v) p {K^\alpha \over (K + q)^\alpha} \1{q \geq 0}
  }_{J_2}
  - b u(\delta v) \left[
    1 - (1 - p) {
      K'^\beta
...
\begin{proof}
  ...
  \begin{eqnarray*}
    \pd{\mathcal U(F)}{\alpha}(\alpha, \beta, K, K', \hat \phi)
    &=& \pd{J_1}{\alpha} + \pd{J_2}{\alpha} > 0
  \end{eqnarray*}
  The 2nd term is easily computed:
  \begin{eqnarray*}
    \pd{J_2}{\alpha} &=& b u(\delta v) p \1{q \geq 0}
    \left(
      {K \over K + q}
    \right)^\alpha
    \ln\left(
  ...
  \item When $q \geq 0$,
    \begin{eqnarray*}
      && \pd{J_1}{\alpha}(\alpha, \beta, K, K', \hat \phi) \\
      &=&
  ...
  \item When $q < 0$, it follows from \eqref{eq:xxie1.0}:
    \begin{eqnarray*}
      && \opd{\alpha}J_1(\alpha, \beta, K, K', \hat\phi) \\
      &=& \opd{\alpha} \left[
        -{\alpha K^\alpha \over \xi} p
        \int_{-\infty}^0
            {
