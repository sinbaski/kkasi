...
\begin{proof}
  ...
  \begin{eqnarray*}
    ...
    &=&
    \int_{-\infty}^\infty
    \ln[C(x)] (1 + b \1{x < q})f(x, \nu) dx
    - b \ln(\delta v) F(q, \nu) \\
    &=& 
    \int_{0}^\infty
    \ln\left[C(x) C(-x)\right]
    f(x, \nu) dx
    +
    b \int_{-\infty}^\infty
    \ln
  ...
  Observe that $\ln(C(x)C(-x))$ is an increasing function:
  \begin{eqnarray*}
    \opd{x}
    \left[
      \ln(C(x) C(-x))
  ...
  $\pd{I_1}{\nu} < 0$. If $b = 0$, i.e.
  $\mathcal U(F, \phi) = \E u(C(X))$, then we have concluded
  $\pd{G(F)}{\nu} < 0$. Otherwise we need to investigate $I_2$:
  \begin{enumerate}
  \item In case $q \leq x_0$, by lemma \ref{lemma:II},
    $\pd{f}{\nu}(x, \nu) > 0$ for $x \leq q < x_0$. Moreover
    \[
    \ln\left[
      C(-x)
      \over
