...
\begin{abstract}
  ...
  market and provide a quantitative description of the market's status.
...
\cite{barndorff:shephard:2001} and Eberlein \cite{eberlein:2001} favor
``semi-heavy'' tails, i.e., tails which are comparable with those of a
gamma \ds . On the other hand, tails of returns have been studied in
great detail in the extreme value community. Among extreme value
specialists there is general agreement that returns $X_t$ have tails
of power-law-type, i.e.,
\beam\label{eq:1}
...
al. \cite{Embrechts1997}, Jansen and de Vries
...
al. \cite{Embrechts1997}, Chapter 3. In this
paper, for the  
...
is an extension of the concept of Disappointment Aversion (DA) of Gul
 \cite{gul1991theory}. The Gul paper derives the DA from first principles (axiomatic).
\par 
...
see Embrechts et al. \cite{Embrechts1997}, de
...
see Embrechts et al. \cite{Embrechts1997},
Resnick \cite{resnick:2007} and Drees et al. 
\cite{drees:resnick:2000}. In Figure~\ref{fig:1},
...
al. \cite{Embrechts1997} for illustrations 
...
For convenience, we denote the scale parameter by $A$.
%% It is the constant $J_*(0)$ in the Taylor expansion \eqref{eq:scale_parameter}.
...
hypothesis is
\begin{hypothesis}
  $\mathcal H_0$: For a given $p$ and all $1 \leq i < j \leq n$,
  $F_i^{-1}(p) = F_j^{-1}(p)$
\end{hypothesis}
while the alternative is $\exists t^* \in [t_0, 1 - t_0]$ such that
\[
\bar F_1^{-1}(p) = \bar F_2^{-1}(p) = \cdots =
\bar F_{\floor{t^* n}}^{-1}(p) \neq
\bar F_{\ceil{t^* n}}^{-1}(p) = \cdots = F_n^{-1}(p)
\]
where $t_0 \in (0, 1/2)$ and $\hat x_p(s, t)$ is the Weissman
estimator applied to the partial series
$X_{\ceil{ns}}, \dots, X_{\floor{nt}}$ using $\ceil{k(t - s)}$
upper-order statistics. See Embrechts, Kl\"uppelberg and T.Mikosch
\cite{Embrechts1997}, p.347.

Hoga proposed the following test statistics:
\[
...
\right\}
\]
Under the null hypothesis and suitable dependence conditions, Hoga
showed that as $n \to \infty$,
\begin{equation}
  \label{eq:Q1_distr}
  Q_1 \overset{d}{\to}
...
500 index. The null hypothesis here is that the extreme quantile
remains the same througout the selected period of time.
\begin{figure}[htb!]
...
\begin{figure}[htb!]
  ...
  \caption{Comparison of the asymptotic distribution with empirical
    distribution of $Q_1$ obtained from Student's t samples. Each t
    sample has the same size as the S\&P sectors' data, namely 1304
    observations.}
  \label{fig:Q1_distr}
\end{figure}
...
generalization GO-GARCH of van der Weide \cite{van2002go}, as
well as the Full Factor GARCH model of Vrontos et al 
...
contributing factors (cf. Mikosch \cite{Mikosch2013} as well
as Andersen, Davis, Kreiss and Mikosch 
...
\cite{bollerslev1990modelling} specifies, for a $d$-dimensional
series $\vec X_t$
\begin{eqnarray}
...
\]
where $Z \overset{d}{=} Z_{i,t}$. It is clear $\kappa_i \neq \kappa_j$
except for special values of $A_i, B_i$ and $A_j, B_j$.

As an extension to the CCC-GARCH model, Jeantheau 
...
theorem (cf. Kesten \cite{Kesten1973}). Thus all series under
...
\label{sec:3}
Routledge and Zin \cite{routledge2010generalized} define the GDA
...
\end{equation}
where $\delta > 0$, $v>0$ and $b > 0$. Equivalently
\begin{equation}
  \label{eq:xxie0}
...
preferences follow Gul's original disappointment aversion.
...
\begin{eqnarray*}
  ...
  \right)
\end{eqnarray*}
% Comparing this expression with \eqref{eq:lower_tail_expansion},
