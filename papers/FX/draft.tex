    ...
    r > -\ln \left\{
      1 - \left[
          {\alpha + 1 \over \alpha}
          {
            ...
            \over
            \left[
              b K^{\alpha+1} \left(1 + {q \over K} \right) + (K + q)^{\alpha + 1}
            \right]
          }
          {
            1 \over K e^{1/\alpha}
...
\begin{proof}
  ...
  \item When $q \geq 0$, we have
    ...
    g = 
      ...
      \right\}^{1/\xi}
    \]
    Assumption \eqref{eq:xxie1.1} ensures $g < e^r - 1$. Hence the
...
\end{proof}
\begin{remark}
  When $q = 0$, the lower-bound \eqref{eq:xxie1.1} on the interest
  rate simplifies to
  \begin{equation}
    \label{eq:xxie8}
    K < {
  ...
  \end{equation}
  The right-hand-side of \eqref{eq:xxie8} is a very high upper bound
  on the scale parameter $K$, as is revealed later in this paper.
\end{remark}
