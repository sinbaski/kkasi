...
\end{minipage}
{\red Needed? The following graph (figure \ref{fig:preference_pareto}) shows how
...
on the values of $\xi$ and $(1 - \phi) e^r / \phi$. {\red Conditions?} Moreover, it
is also easily checked that, when $x \in (0, K(e^{1/\alpha} - 1)]$,
the {\red density?} function in the integral of \eqref{eq:ipf} increases
with $\alpha$; when $x \in (K(e^{1/\alpha} - 1), \infty)$ it 
decreases with $\alpha$. Following the arguments for Lemma~\ref{thrm:I},
in particular  using Lemma \ref{lemma:I}, it can be seen that
$\wt u_{\rm max}(\alpha, K)$ 
increases/decreases with $\alpha$ when $U_{\text{all}}(x)$ 
decreases/increases.
