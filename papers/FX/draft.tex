...
The right-hand-side of \eqref{eq:functional} is a functional of $\phi$
and the distribution function of $X$ via \eqref{eq:xxie1}. For convenience
we denote it $\mathcal U(\cdot, \cdot)$, i.e.
\[
\mathcal U(F, \phi)
= 
...
\begin{theorem}
  \label{thrm:I}
  \[
  \pd{G(F)}{\alpha} (\alpha, \beta, K, K') > 0,
  \quad
  \pd{G(F)}{K} (\alpha, \beta, K, K') < 0
  \]
\end{theorem}
...
\begin{proof}
  ...
  We have
  \[
  G(F)(\alpha, \beta, K, K') = \mathcal U(F, \hat\phi)(\alpha, \beta, K, K')
  \]
  It follows
  \begin{eqnarray}
    \pd{G(F)}{\alpha}
    &=&
    \pd{\mathcal U(F, \phi)}{\alpha}(\alpha, \beta, K, K', \hat\phi)
    \nonumber \\
    && + \pd{\mathcal U(F, \phi)}{\phi}(\alpha, \beta, K, K', \hat\phi)
  ...
  \begin{equation}
    \label{eq:xxie4}
    \pd{\mathcal U(F, \phi)}{\phi}(\alpha, \beta, K, K', \hat \phi) = 0
  \end{equation}
  ...
  \begin{eqnarray*}
    \pd{\mathcal U(F, \phi)}{\alpha}(\alpha, \beta, K, K', \hat \phi) &>& 0
  \end{eqnarray*}
  As for $\pd{G(\alpha, \beta, K, K')}{K}$, by the same argument as for
  $\pd{G(\alpha, \beta, K, K')}{\alpha}$, it suffices to show
  \[
  \pd{\mathcal U(F, \phi)}{K}(\alpha, \beta, K, K', \hat\phi) < 0
  \]
  We have
  \[
  \pd{\mathcal U(F, \phi)}{K}(\alpha, \beta, K, K', \hat\phi)
  = p \opd{K} \E \left[
    u((1 - \phi) e^r + \phi e^X)
    | X < 0
  \right] 
...
\begin{proof}
  ...
  \end{eqnarray*}
  where $C(\cdot)$ is defined in \eqref{eq:xxie1}. In the following we show
  \[
  \pd{I_1}{\nu} > 0, \quad
  \pd{I_2}{\nu} > 0, \quad
  \pd{I_3}{\nu} > 0
  ...
  \begin{eqnarray*}
    ...
    f(x, \nu) dx
  \end{eqnarray*}
  Observe that $u(C(x)) + u(C(-x))$ is a decreasing function:
  \begin{eqnarray*}
    \opd{x}
    \left[
      u(C(x)) + u(C(-x))
    \right] &=& u'(C(x)) C'(x) - u'(C(-x)) C'(-x)
  \end{eqnarray*}
