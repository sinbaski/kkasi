...
N = 350;
pdf(sprintf("EigenRatioSP500_loglog_%d_shown.pdf", N));
...
plot(log(E$values[2:p]/E$values[1:p-1]), type="b", pch=20, xlim=c(1, N), ylim=c(-2.1, 0),
     xlab=expression(log(i)), ylab=expression(log(lambda[(i+1)]/lambda[(i)])));
I <- 1:(N-1);
q = 0.99;
...
Q[, 3] = (1-q)^(2/I);
## expected <- I / (I+2);
lines(log(I), log(Q[, 2]), col="#0000FF", type="l");
lines(log(I), log(Q[, 3]), col="#FF0000");
lines(log(I), log(Q[, 1]), col="#00FF00");
dev.off();
