...
% plot(variance, A, '+-');
% grid on
% legend('empirical \lambda_{min}', '\lambda_{min} by linear approx.', 'Location', 'Northwest');
% title(sprintf('q=%.1f', q));

% figure;
plot(variance, B, '+-');
grid on
legend('empirical \lambda_{max}', '\lambda_{max} by linear approx.', 'Location', 'Northwest');
title(sprintf('q=%.1f', q));
