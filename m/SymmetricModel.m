...
f = @(x, s, evbar) 1/evbar*...
    ...
    erfc(1/(s*sqrt(2)) * log(abs(x./evbar)/sqrt(log(4))) + s/sqrt(2));

p = mle(r, 'pdf', f, 'start', [1, std(r)], 'lowerbound', [1e-4, 1e-4]);
% [y, x] = ecdf(r);
% y1 = F(x, sig);
% plot(x, y, x, y1, 'r');
...
% 'r');
[y, x] = ecdf(r);
y1 = retcdf_symmetric(x, p(1), p(2));
I = x < 0;
% I = x < -2*sigma & x > -3*sigma;
% I = x > 2*sigma & x < 3*sigma;
% I = logical(ones(1, length(x)));

plot(log(-x(I)), log(y(I)), log(-x(I)), log(y1(I)), 'r');
grid on
legend('ln(-x) vs. ln(ecdf(x))', 'ln(-x) vs. ln(F(x))', 'Location', 'Southwest');
title('probability distribution function');
xlabel('ln(-x) with $x < 0$', 'Interpreter', 'latex', ...
       'Fontsize', 14);
ylabel('ln(F(x)) and ln(ecdf(x))');
